...
def send_datasets(domain_name, send_now=False, send_date=None):
    ...
    for dataset_map in dataset_maps:
        if send_now or dataset_map.should_send_on_date(send_date):
            conn = dataset_map.connection_settings or dhis2_conn
            if not conn:
                continue
            ...
            requests = Requests(
                domain_name,
                # TODO: When Dhis2Connection is removed, don't forget this:
                conn.url if hasattr(conn, 'url') else conn.server_url,
                conn.username,
